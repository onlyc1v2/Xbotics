...
from .cfg import AnymalCEnvCfg, AnymalCEnvTerrainCfg
...
@registry.env("anymal_c_navigation_terrain", "np")
class AnymalCTerrainEnv(AnymalCEnv):
    """复杂地形导航环境 - 继承平地版本，覆盖地形相关的逻辑"""
    _cfg: AnymalCEnvTerrainCfg
    
    def __init__(self, cfg: AnymalCEnvTerrainCfg, num_envs: int = 1):
        super().__init__(cfg, num_envs=num_envs)
        
        # 地形特定初始化
        self._last_base_height = np.zeros(num_envs, dtype=np.float32)  # 追踪机器人高度，检测坠落
    
    def _extract_terrain_info(self, data: mtx.SceneData):
        """从高度场获取地形信息（局部）"""
        pose = self._body.get_pose(data)
        root_pos = pose[:, :3]  # [num_envs, 3]
        
        # 根据地形高度字段计算坡度（简化：使用邻近点高度差推测）
        # 在实际应用中，可通过ray casting或直接查询高度字段
        # 这里使用相邻点高度差近似
        
        num_envs = data.shape[0]
        local_slope = np.zeros((num_envs, 2), dtype=np.float32)  # [dx_slope, dy_slope]
        
        # 简单近似：记录上次高度，计算变化率
        height_change = root_pos[:, 2] - self._last_base_height
        self._last_base_height = root_pos[:, 2].copy()
        
        return root_pos, height_change
    
    def update_state(self, state: NpEnvState):
        """复杂地形版本的状态更新 - 添加地形特定的奖励和终止条件"""
        # 调用父类的update_state获取基础观测和奖励
        state = super().update_state(state)
        
        # 获取地形信息
        data = state.data
        root_pos, height_change = self._extract_terrain_info(data)
        
        # 添加地形特定的终止条件：坠落检测
        # 如果机器人在一个step内下降超过阈值，认为掉下了悬崖
        max_fall = self._cfg.fall_height_threshold
        fell_off = height_change < -max_fall
        
        state.terminated = np.logical_or(state.terminated, fell_off)
        
        # 添加地形特定的奖励
        fall_penalty = np.where(fell_off, -20.0, 0.0)
        
        # 爬坡奖励：上升时给予小额奖励（鼓励向上运动）
        climbing_bonus = np.where(
            height_change > 0.02,  # 上升超过2cm
            height_change * 5.0,    # 上升越多奖励越多
            0.0
        )
        
        # 修改reward（添加地形特定项）
        state.reward = state.reward + fall_penalty + climbing_bonus * 0.1
        
        return state
    
    def reset(self, data: mtx.SceneData, done: np.ndarray = None) -> tuple[np.ndarray, dict]:
        """复杂地形版本的reset - 根据地形高度调整起始高度"""
        cfg = self._cfg
        num_envs = data.shape[0]
        
        # 生成机器人的初始位置（在世界坐标系中）- 略低，会下沉到地形表面
        pos_range = cfg.init_state.pos_randomization_range
        robot_init_x = np.random.uniform(
            pos_range[0], pos_range[2],
            num_envs
        )
        robot_init_y = np.random.uniform(
            pos_range[1], pos_range[3],
            num_envs
        )
        robot_init_pos = np.stack([robot_init_x, robot_init_y], axis=1)  # [num_envs, 2]
        ...
        # 调用父类reset逻辑；这里传入done是为了让上层记录信息
        obs, info = super().reset(data, done)
        
        # 由于是复杂地形，需要让机器人"掉落"到地形表面。
        # 原始实现通过运行几十步物理模拟让重力自然生效，但
        # 当只重置部分环境时（data 可能只有 160 个子场景）
        # 对这一小批次调用 self._model.step 会导致底层求解器
        # 产生奇异惯性矩阵并 panic。为简单起见，只有在
        # 全部环境同时被 reset 的情况下才运行多步仿真。
        if (done is None) or (done.all()):
            for _ in range(50):  # 运行50个 step 让机器人适应地形高度
                self._model.step(data)
        # 不论是哪种情况都执行一次前向运动学，以便计算初始高度
        self._model.forward_kinematic(data)
        ...
        # 重置高度追踪。注意当只重置部分环境时，`data` 和
        # `root_pos` 只包含那些 done=True 的环境，且 `done` 传
        # 递了对应的全局布尔掩码。我们不能简单地将整个
        # _last_base_height 替换为新数组，否则会在下一次
        # step 时导致广播错误（长度不匹配）。
        if done is None:
            # 如果没有传递掩码，则重置所有环境
            self._last_base_height = root_pos[:, 2].copy()
            ...
            # 仅更新被重置的环境槽位
            self._last_base_height[done] = root_pos[:, 2].copy()
